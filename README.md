...
| Version | Changes |
|---|---|
| 1.7.3 | **Configurable good / bad trend colors.** The trend arrow, delta text, and recent-portion sparkline color were hardcoded green (`#22C55E`) for the good direction and red (`#EF4444`) for the bad direction. Two new formatter options in the **Trend** section — `trendColorGood` and `trendColorBad` — let you pick any hex color for each. Defaults preserve the pre-1.7.3 behaviour. Useful when a dashboard has a brand palette (Splunk blue, corporate teal, etc.) or an accessibility requirement (colorblind-safe alternatives to red/green). |
| 1.7.2 | **Split-color sparkline with compare-point marker.** When both Show Trend and Show Sparkline are on, the sparkline now visually communicates *where the comparison anchor sits on the timeline*. The historical portion (index 0 → compare-back index) draws in a muted base color; the recent portion (compare-back index → last row) draws in the trend color (green when up_good rising, red when up_bad rising, etc.). A thin semi-transparent vertical line marks the split X position and a small filled dot sits on the polyline at the exact split point. New option `sparklineSplitAtCompare` (yes/no, default yes) toggles this behaviour — set to `no` for a single-color sparkline. Inspired by the split-color pattern in the Business Process Flow custom viz. |
| 1.7.1 | **Sparkline.** Three new options in the **Trend** formatter section: `showSparkline` (yes/no), `sparklineColor` (color picker, `auto` follows the trend color; also accepts hex), `sparklineStyle` (line or area with translucent fill). Uses the same time-series already fetched by `getInitialDataParams.count=200` — no additional data cost. The sparkline reserves ~28 % of the value area at the bottom; when combined with the trend arrow, trend text sits at ~22 % and sparkline at ~28 %. Sample 5 tiles updated to show the sparkline live — three tiles use `auto` color (inherits trend color / threshold band), one uses an explicit hex. |
